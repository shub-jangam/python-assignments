...
def main():
    ...
    if(len(sys.argv) == 2):
        if((sys.argv[1] == "--h") or (sys.argv[1] == "--H")):
            ...
            print("ScriptName.py  Process Name")
            print("Please provide valid ProcessName")
        else:
            logFileName = createlogFile(sys.argv[0],sys.argv[0],FolderName= "")
            processName = sys.argv[1].lower() + ".exe"
            processList = GetProcessInfoByName(processName)
            if len(processList)>0:
                ...
                appendMessage(logFileName,"No Active process found for "+ processName)       
    else:
