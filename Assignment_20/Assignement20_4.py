...
def main():
    ...
    if(len(sys.argv) == 2):
        if((sys.argv[1] == "--h") or (sys.argv[1] == "--H")):
            ...
            appendMessage(logFileName , f"Execution Started  {start_time}" )
            DirectoryDublicateRemoval(sys.argv[1],logFileName)      
            end_time = time.perf_counter()  
            appendMessage(logFileName , f"Execution ended  {end_time}" )
            elapsed_time = end_time - start_time  
            appendMessage(logFileName , f"total time  {elapsed_time}" )
