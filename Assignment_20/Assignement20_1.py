...
def DirectoryCheckSum(DirectoryName,logFilename):
    ...
    for foldername, subFoldenames, FilesNames in os.walk(DirectoryName):
        for fname in FilesNames:
            ...
            chckSum=calculate_checksum(sourcePath)
            appendMessage(logFilename ,f"file {fname} have chcksum value {chckSum}" )
...
def main():
    ...
    if(len(sys.argv) == 2):
        if((sys.argv[1] == "--h") or (sys.argv[1] == "--H")):
            ...
            DirectoryCheckSum(sys.argv[1],logFileName)          
